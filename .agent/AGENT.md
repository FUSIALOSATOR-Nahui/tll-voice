...
Этот документ описывает технологический стек, архитектурные правила и стандарты качества для инкрементального развития TLL-Voice v0.5. Все будущие ИИ-агенты обязаны строго придерживаться этих инструкций.
...
- `adapter.get_foreground_window()` — получение активного окна ОС для трекинга фокуса
- `adapter.set_foreground_window(window_id)` — возврат фокуса в исходное окно
...
**Только если Автор явно попросил перейти на "Бэклог задач", то: смотри [backlog_list.md](.agent/backlog_list.md)**
